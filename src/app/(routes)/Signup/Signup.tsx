...
function Signup() {
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    if (data.userName && data.email && data.password) {
      ...
      // Append the values from the data object to the FormData object
      formData.append("userName", data.userName);
      formData.append("email", data.email);
      formData.append("password", data.password);

      // Pass the FormData object to your signup logic
      const result = await signupLogic(formData);
          ...
          <div className=" w-[80%] sm:w-[50%] m-auto h-full ">
            <div className=" py-9">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className=" ">
                <input
                  className={`w-full py-3 mt-3 text-xs px-2 rounded-md ${
                ...
                />
                {errors.userName && (
                  <p className=" text-[11px]  text-red-500 pt-2 px-2 ">
                ...
                <button
                  type="submit"
                  className="bg-black text-white w-full py-3 mt-8 mb-4 font-semibold text-sm  rounded-md"
                >
                  Sign up
                </button>
              </form>
              ...
              >
                <img
                  ...
                  alt=""
                />{" "}
                <p className="inline px-2">Sign in with Google</p>
              </button>
            </div>
          </div>
        </div>
